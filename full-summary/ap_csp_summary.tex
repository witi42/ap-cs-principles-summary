...
\definecolor{codeblue}{rgb}{0,0,0.9} % New dark blue for keywords
\definecolor{codeorange}{rgb}{0.8,0.4,0} % New orange for function names
\definecolor{commentcolor}{rgb}{0.5,0.5,0.5} % Grey for comments
\definecolor{outputback}{rgb}{0.9,0.95,0.9} % Light green for output boxes

% --- Define AP CSP pseudocode language ---
\lstdefinelanguage{APCSP}{
  morekeywords={IF, ELSE, FOR, EACH, IN, REPEAT, UNTIL, PROCEDURE, RETURN, DISPLAY, INPUT, APPEND, REMOVE, INSERT, AND, OR, NOT},
  morecomment=[l]{//},
  morecomment=[s]{/*}{*/},
  sensitive=false,
  morestring=[b]",
...
\lstdefinestyle{mystyle}{
    ...
    commentstyle=\color{commentcolor}\bfseries, % Bold red comments - very obvious change
    keywordstyle=\color{magenta},
    numberstyle=\tiny\color{codegray},
    stringstyle=\color{codepurple},
    identifierstyle=\color{black}, % Default color for identifiers
    emphstyle=\color{codeorange}, % Orange for emphasized elements
    ...
    showtabs=false,
    tabsize=2,
    mathescape=true, % Added to allow math mode in listings
    literate={<-}{$\leftarrow$}2 % Auto-replace with AP CSP symbols
             {>=}{$\geq$}1
             {<=}{$\leq$}1
             {!=}{$\neq$}1,
}

% Apply style globally and set the default language
\lstset{style=mystyle,language=APCSP}
...
% Output box for showing program output
...
\begin{titlepage}
    ...
    \vspace{1cm}
    {\large\href{https://rowi.dev/}{Robin Wiethüchter}\par}
    \vspace{1cm}
    {\large\today\par}
...
\begin{itemize}
\item \textbf{Declaration/Assignment}: In AP Pseudocode, variables are often declared implicitly when they are first assigned a value using the assignment operator ($\leftarrow$). In other languages, you might need to declare the type first.
...
\end{itemize}
\begin{lstlisting}[language=APCSP, label={lst:math_expr_detail}, caption={AP Pseudocode: Math Expressions}]
...
\midrule
\hlc[hlyellow]{==} & \hlc[hlyellow]{=} & Equal to \\
...
\end{lstlisting}
\begin{outputbox}
\texttt{Checking score because homework is complete...}\\
\texttt{A}
\end{outputbox}
...
\label{sec:lists}
Lists (or arrays in some languages) are ordered collections of items (elements). They allow you to store multiple values under a single variable name.

\subsection*{List Indexing: AP CSP Uses 1-Based Indexing}
\textbf{Important:} AP Pseudocode uses \textbf{1-based indexing}, meaning the first element is at index 1. This differs from most programming languages (like JavaScript and Python) which use 0-based indexing where the first element is at index 0.

% Visual representation of a list with indexes
\begin{figure}[h!]
    \centering
    \begin{tikzpicture}
        % List elements
        \foreach \i/\val in {0/"Apple",1/"Banana",2/"Cherry",3/"Date"} {
            \draw (\i*2.2,0) rectangle ++(2,1);
            \node at (\i*2.2+1,0.5) {\val};
        }
        % AP CSP indexes (1-based)
        \node at (-0.8,-0.5) {\textbf{AP CSP:}};
        \foreach \i/\idx in {0/1,1/2,2/3,3/4} {
            \node[text=blue] at (\i*2.2+1,-0.5) {\texttt{[\idx]}};
        }
        % Most languages indexes (0-based)
        \node at (-0.8,-1.2) {\textbf{JS/Python:}};
        \foreach \i/\idx in {0/0,1/1,2/2,3/3} {
            \node[text=red] at (\i*2.2+1,-1.2) {\texttt{[\idx]}};
        }
    \end{tikzpicture}
    \caption{List indexing comparison: AP CSP starts at 1, while JavaScript and Python start at 0}
    \label{fig:list-indexing}
\end{figure}
...
\begin{lstlisting}[language=APCSP, caption={AP Pseudocode: Creating and Accessing Lists}]
...
AP Pseudocode provides several built-in operations for manipulating lists:

\begin{tabularx}{\textwidth}{>{\ttfamily}l X}
\toprule
\textbf{Operation} & \textbf{Description} \\
\midrule
APPEND(list, value) & Adds \texttt{value} to the end of \texttt{list} \\
INSERT(list, i, value) & Inserts \texttt{value} at index \texttt{i}, shifting elements to the right \\
REMOVE(list, i) & Removes the element at index \texttt{i}, shifting elements to the left \\
LENGTH(list) & Returns the number of elements in \texttt{list} \\
\bottomrule
\end{tabularx}

\begin{lstlisting}[language=APCSP, caption={AP Pseudocode: List Operations}]
colors <- ["Red", "Blue"]

APPEND(colors, "Green")
DISPLAY(colors)

INSERT(colors, 2, "Yellow")
DISPLAY(colors)
...
\end{lstlisting}
\begin{outputbox}
\texttt{["Red", "Blue", "Green"]}\\
\texttt{["Red", "Yellow", "Blue", "Green"]}\\
\texttt{["Yellow", "Blue", "Green"]}\\
\texttt{3}
\end{outputbox}

\subsection*{Iteration with FOR EACH}
The \texttt{FOR EACH} loop iterates through each element in a list sequentially. The loop variable takes on the value of each element (not the index).

\begin{lstlisting}[language=APCSP, caption={AP Pseudocode: FOR EACH Loop}]
scores <- [85, 92, 78, 95]
sum <- 0
...
\textbf{Note:} In a \texttt{FOR EACH} loop, the loop variable (\texttt{score} in the example above) holds the \textit{value} of the current element, not its index. If you need the index, you would use a counter variable with a \texttt{REPEAT} loop instead.
...
\section{Language Comparison: AP CSP vs JavaScript vs Python}
\label{sec:language_comparison}
Different programming languages use different syntax for the same concepts. Here is a comparison of common operations:

\subsection*{Assignment and Basic Operations}
\begin{tabularx}{\textwidth}{l >{\ttfamily\small}l >{\ttfamily\small}l >{\ttfamily\small}l}
\toprule
\textbf{Operation} & \textbf{AP CSP} & \textbf{JavaScript} & \textbf{Python} \\
\midrule
Assignment & x $\leftarrow$ 5 & x = 5 & x = 5 \\
Equality & a = b & a == b & a == b \\
Not equal & a $\neq$ b & a != b & a != b \\
Greater/Less & >, <, $\geq$, $\leq$ & >, <, >=, <= & >, <, >=, <= \\
Logical AND & a AND b & a \&\& b & a and b \\
Logical OR & a OR b & a || b & a or b \\
Logical NOT & NOT a & !a & not a \\
Modulus & a MOD b & a \% b & a \% b \\
\bottomrule
\end{tabularx}
...
\begin{tabularx}{\textwidth}{l >{\ttfamily\small}l >{\ttfamily\small}l >{\ttfamily\small}l}
\toprule
\textbf{Concept} & \textbf{AP CSP} & \textbf{JavaScript} & \textbf{Python} \\
\midrule
Define & PROCEDURE name() & function name() \{\} & def name(): \\
With params & PROCEDURE f(a, b) & function f(a, b) \{\} & def f(a, b): \\
Return value & RETURN(value) & return value & return value \\
Call & name() & name() & name() \\
\bottomrule
\end{tabularx}

\textbf{Note:} AP CSP uses the term ``procedure'' for all callable blocks of code. JavaScript and Python distinguish between functions (which return values) and procedures/methods (which may not), but in practice both terms are often used interchangeably.

\subsection*{Lists and Indexing}
\begin{tabularx}{\textwidth}{l >{\ttfamily\small}l >{\ttfamily\small}l >{\ttfamily\small}l}
\toprule
\textbf{Operation} & \textbf{AP CSP} & \textbf{JavaScript} & \textbf{Python} \\
\midrule
Create list & [1, 2, 3] & [1, 2, 3] & [1, 2, 3] \\
First index & \textbf{1} & \textbf{0} & \textbf{0} \\
Access first & list[1] & list[0] & list[0] \\
Access last & list[LENGTH(list)] & list[list.length-1] & list[-1] \\
Length & LENGTH(list) & list.length & len(list) \\
Append & APPEND(list, val) & list.push(val) & list.append(val) \\
Remove at i & REMOVE(list, i) & list.splice(i-1, 1) & list.pop(i) \\
Insert at i & INSERT(list, i, val) & list.splice(i-1, 0, val) & list.insert(i, val) \\
\bottomrule
\end{tabularx}

\textbf{Important:} Remember that AP CSP uses \textbf{1-based indexing} while JavaScript and Python use \textbf{0-based indexing}. This is a common source of errors when translating between them.

\subsection*{Loops}
\begin{tabularx}{\textwidth}{l >{\ttfamily\small}l >{\ttfamily\small}l >{\ttfamily\small}l}
\toprule
\textbf{Loop Type} & \textbf{AP CSP} & \textbf{JavaScript} & \textbf{Python} \\
\midrule
Repeat n times & REPEAT n TIMES & for(let i=0; i<n; i++) & for i in range(n): \\
For each & FOR EACH x IN list & for(let x of list) & for x in list: \\
Repeat until & REPEAT UNTIL(cond) & while(!cond) \{\} & while not cond: \\
\bottomrule
\end{tabularx}
