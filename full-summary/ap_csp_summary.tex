    ...
    \item \textbf{Countermeasures}:
        \begin{itemize}
            \item \textbf{Authentication}: Verifying identity (passwords, biometrics).
                ...
                \end{itemize}
            \item \textbf{Encryption}: Scrambling data (plaintext) into an unreadable format (ciphertext) using an algorithm and a secret key, so it's unintelligible without the key. Used for secure communication (HTTPS, WPA2) and data storage.
                \begin{itemize}
            ...
            \begin{figure}[h!]
                \centering
                \begin{tikzpicture}[node distance=2cm and 1cm, every node/.style={draw, rounded corners, align=center}]
                    % Nodes
                    \node (Alice) {Alice};
                    \node (Bob) [right=4cm of Alice] {Bob};
                    \node (Message) [below left=0.5cm and -0.5cm of Alice] {Plaintext\Message};
                    \node (Encrypt) [right=of Message] {Encrypt using\Bob's Public Key};
                    \node (Ciphertext) [right=of Encrypt] {Ciphertext\(Unreadable)};
                    \node (Decrypt) [right=of Ciphertext] {Decrypt using\Bob's Private Key};
                    \node (Received) [below right=0.5cm and -0.5cm of Bob] {Plaintext\Message};
                    
                    % Arrows
                    \draw[->] (Message) -- (Encrypt);
                    \draw[->] (Encrypt) -- (Ciphertext);
                    \draw[->] (Ciphertext) -- (Decrypt);
                    \draw[->] (Decrypt) -- (Received);
                    
                    % Key indication (simplified)
                    \node (PubKey) [above=0.2cm of Encrypt, draw=none] {Bob's Public Key \$\rightarrow$}; 
                    \node (PrivKey) [above=0.2cm of Decrypt, draw=none] {Bob's Private Key \$\rightarrow$};
                \end{tikzpicture}
